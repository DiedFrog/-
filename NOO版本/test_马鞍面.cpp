...
void main() {
    ...
    // 双曲抛物面（马鞍面）参数
    float scale = 0.001; // 控制马鞍面的弯曲程度
    float intensity = 50.0; // 控制马鞍面的强度
    
    // 计算马鞍面效果
    // 双曲抛物面公式: z = x² - y²
    // 这里我们用 y 作为高度，x 和 z 作为水平坐标
    float r_xz = length(p.xz);
    float hyperbolicHeight = intensity * (p.x * p.x - p.z * p.z) * scale;
    ...
    if (r_xz > 0.0) {
        // 保持水平位置不变，只修改垂直位置
        warpedPos.y = p.y + hyperbolicHeight;
        
        // 可选的：为了更明显的效果，也可以稍微向内收缩
        // float shrinkFactor = 1.0 / (1.0 + r_xz * r_xz * 0.001);
        // warpedPos.xz *= shrinkFactor;
    }
...
const char* groundVertexShaderSource = R"(
    ...
    void main() {
        ...
        // 双曲空间（马鞍面）参数
        float hyperbolicScale = 0.001; // 马鞍面缩放系数
        float hyperbolicIntensity = 50.0; // 马鞍面强度
        
        // 计算双曲抛物面（马鞍面）效果
        float hyperbolicHeight = hyperbolicIntensity * (p.x * p.x - p.z * p.z) * hyperbolicScale;
        
        vec3 warpedPos = p;
        
        // 应用双曲空间变换
        float r = length(p.xz);
        float maxRadius = 100.0; // 最大渲染半径
        
        if (r < maxRadius) {
            // 基础马鞍面变形
            warpedPos.y = -1.0 + hyperbolicHeight;
            
            // 可选的：添加径向收缩以增强透视感
            float radialFactor = 1.0 / (1.0 + r * r * 0.0005);
            warpedPos.xz *= radialFactor;
            
            // 可选：添加水平方向的轻微弯曲
            // float horizontalBend = sin(r * 0.1) * 0.1;
            // warpedPos.y += horizontalBend;
        } else {
            // 超出范围的顶点移到远处
...
int main()
{
    ...
    while(!glfwWindowShouldClose(window)){
        ...
        for(int i=0;i<5;i++){
            ...
            cubeModel[i] = glm::scale(cubeModel[i],glm::vec3(3.0f,3.0f,1.0f));

...
void main() {
    ...
    if (r > 0.0) {
        ...
        // 垂直位置：基于 y 加上球面高度偏移
        // R * cos(angle) - R 得到从原点下垂的弧度
        float heightOffset = R * cos(angle) - R;
        warpedPos.y = p.y + heightOffset;
    }
...
const char* groundVertexShaderSource = R"(
    ...
    void main() {
        ...
        float angle = safe_r / R;
        
        if (angle < 1.57) {
            // 使用一个更稳健的映射：当 r 趋近 0 时，s/r 趋近 1.0
            // 直接计算缩放因子
            float s_factor = (R * sin(angle)) / safe_r;
            
            warpedPos.x = p.x * s_factor;
            warpedPos.z = p.z * s_factor;
            warpedPos.y = -1.0 + (R * cos(angle) - R);
        } else {
